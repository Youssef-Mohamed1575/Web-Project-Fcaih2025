document.addEventListener("DOMContentLoaded", () => {
  ...
  input.addEventListener("keyup", function () {
    //  key up function كل مرة المستخدم يضغط على زر في الكيبورد وهو بيكتب في خانة البحث، شغّل الكود اللي جوا
    let filter = this.value.toLowerCase(); //يتحول كل الحروف ل lower
    let items = document.querySelectorAll(".menu-item"); //كل العناصر اللي واخده ال class.item يتتسجل في عنصر items
    items.forEach((item) => {
      // يتعدي علي كل عنصر من المشروبات
